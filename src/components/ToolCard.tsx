...
const ToolCard: React.FC<ToolCardProps> = ({ tool, className }) => {
  return (
    <Link
      to={tool.path}
      className={cn(
        "relative overflow-hidden glass rounded-xl p-4 sm:p-6 transition-all duration-300",
        "hover:shadow-glass-lg hover:-translate-y-1",
        "group flex flex-col items-center justify-center text-center h-full min-h-[140px] sm:min-h-[160px]",
        className
      )}
    >
      <div 
        className={cn(
          "p-2 sm:p-3 rounded-full mb-3 sm:mb-4 transition-transform duration-300 group-hover:scale-110", 
          tool.color
        )}
      >
        <tool.icon className="w-5 h-5 sm:w-6 sm:h-6" />
      </div>
      <h3 className="font-medium text-sm sm:text-lg mb-1">{tool.name}</h3>
